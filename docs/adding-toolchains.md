...
The built-in npm requirement first reuses npm, then tries mise when it is already installed, and finally uses curl plus nvm to install persistent Node under `FWD_TOOL_PREFIX`. From the synced project directory, the nvm fallback runs `nvm install` and `nvm use` against `.nvmrc` when that file exists, or selects the latest LTS otherwise. It does not edit the remote user's profiles; it links the selected Node version's executable commands into the existing fwd bin directory so later non-interactive probes and dependent installers see the same runtime.
