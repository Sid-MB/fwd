...
A target is a named, reusable backend configuration—not a running machine or session. It describes how to provision or reach compute, including values such as an SSH alias, RunPod compute type, Lambda region policy and instance type, or Slurm allocation. RunPod setup loads searchable GPU-type and persistent-storage-datacenter catalogs from `runpodctl`, with free-text fallback only when discovery fails. Lambda defaults to automatic exact-region selection from live capacity; `preferred_regions` entries are ordered exact codes or prefixes, retained filesystems pin replacement instances to their original region, and setup searches and validates exact instance-type and registered SSH-key names against Lambda's live catalog. Closed provider fields use a temporary menu below the prompt: typing filters every live value, Tab/Shift-Tab and arrow keys navigate matches, Enter selects the highlighted value, and the menu is cleared afterward. Interactive Lambda setup uses fwd's shared credential component: one hidden prompt automatically distinguishes a pasted API key from quoted or unquoted path-looking input, rejects unreadable paths, and saves either a private managed secret or a reusable external-file reference under `~/.fwd/credentials`. Setup matches registered provider public keys to usable local public/private pairs, persists the matching private `key_path`, rejects unmatched registered keys, and can upload public halves read from adjacent `.pub` files or derived locally from any valid private key under `~/.ssh`, including `.pem`. `ssh-add -L` annotates agent-loaded matches, but agent-only identities are not treated as persistable because OpenSSH cannot export their private paths. SSH-key uploads require a structurally valid OpenSSH public blob, while a separate API egress guard rejects private-key blocks and payload fields before request construction without echoing rejected material. Inspect configured targets and their source files with `fwd config`; inspect current machine strings and defaults with `fwd up --machines` or `fwd up TARGET --machines`; inspect all valid configuration fields with `fwd config --schema` or `fwd config --example BACKEND`. Add a target interactively with `fwd setup`, or select its form directly with `fwd setup BACKEND` (equivalent to `--backend BACKEND`, and neither form repeats the backend question) plus the required flags shown by `fwd setup --help`.
...
Interactive `fwd up` provisioning also reserves Ctrl-B after two seconds. It backgrounds only the local launch viewer while a detached worker continues provider polling, SSH readiness, sync, bootstrap, and remote tmux startup, with progress retained in the printed private `~/.fwd/logs/launch-*.log`. Ctrl-C remains cancellation and is forwarded to the worker so launch-owned resource cleanup still runs. Use `fwd ls` to inspect a background launch once provider state appears and `fwd attach NAME` when it is ready.
