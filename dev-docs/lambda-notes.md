...
Create an API key in the Lambda Cloud console and export it locally as `LAMBDA_API_KEY`, or enter it during interactive setup. The one hidden shared credential prompt accepts either the key or a quoted/unquoted UTF-8 key-file path, rejects unreadable path-looking input, and strips values when read. Pasted keys are copied to the mode-600 `~/.fwd/credentials/LAMBDA_API_KEY.secret`; external paths are retained in the mode-600 `~/.fwd/credentials/LAMBDA_API_KEY.path` reference so later commands reread the original file. The containing directory is mode 700, and an environment value overrides either saved source. Setup lists and validates registered public-key names through `GET /ssh-keys`. It discovers every valid private-key file directly under `~/.ssh`, uses adjacent `.pub` files when present, and otherwise derives the public value locally with `ssh-keygen -y`; this includes `.pem` and extensionless keys. It also calls `ssh-add -L` to annotate loaded identities and accepts an agent comment as an external key path only after verifying that file's derived identity. Agent-only identities have no exportable private path and cannot be saved as `key_path`. Password-protected keys need an adjacent readable `.pub` because setup does not solicit private-key passphrases. Upload accepts only a structurally valid one-line OpenSSH public-key blob whose embedded wire algorithm matches its text prefix, then removes its optional comment before transmission. Independently, every Lambda API payload passes an egress guard that rejects recognizable private-key blocks and private-key fields before request construction; errors are deliberately generic and never echo rejected material. Only public values reach Lambda Cloud. When SSH cannot select the matching private key through that path, the agent, or OpenSSH config, set `key_path` manually.
...
Use `fwd setup lambda` to query `GET /regions` through a searchable exact-region prompt, then load every offered instance type from `GET /instance-types` into a temporary filtered menu with Tab/Shift-Tab and arrow-key navigation, exact-name validation, pricing, and current regional availability. The prompt stays above the menu and the menu clears after selection. Setup also discovers registered SSH keys and images. Registered public keys are matched by algorithm and key blob to usable local file-backed identities discovered through adjacent public keys, local derivation, and verified SSH-agent path comments; setup persists the matching private `key_path`, rejects an unmatched provider key, and offers to upload a usable local public key instead. The default `region = "auto"` makes fwd select an exact capacity-bearing region at
